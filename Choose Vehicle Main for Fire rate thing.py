...
def create_second_window():
    ...
    with open(f'{WT_directory}\\UserMissions\\Ask3lad\\ask3lad_testdrive_Instant_Reload.blk', 'r') as f:
        lines = f.readlines()

    #Prints the current Vehicle
    CurrentTank = lines[252]
    ...
    def take_input():
        if selected_Tank.get() == selected_TankDefault or selected_Tank.get() == '':   #if the tank = nothing or the default value do nothing
            ...
            lines[252] = 'weapons:t="{}_default"\n'.format(ChosenTank)
            with open(f'{WT_directory}\\UserMissions\\Ask3lad\\ask3lad_testdrive_Instant_Reload.blk', 'r') as f:
                file_contents = f.readlines()
            file_contents[252] = lines[252]
            with open(f'{WT_directory}\\UserMissions\\Ask3lad\\ask3lad_testdrive_Instant_Reload.blk', 'w') as f:
